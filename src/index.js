...
client.once("ready", async () => {
  ...
  client.user.setActivity("Naoxy Bot | /help", { type: 3 });

  const rest = new REST().setToken(TOKEN);
  try {
    console.log("📡 Enregistrement des slash commands...");
    await rest.put(Routes.applicationGuildCommands(CLIENT_ID, "1229533200989032528"), { body: slashCommandsData });
    console.log(`✅ ${slashCommandsData.length} slash commands enregistrées.`);
  } catch (e) { console.error(e); }
});
...
client.on("interactionCreate", async interaction => {
  ...
  catch (e) { console.error(e); interaction.reply({ content: "Une erreur est survenue.", ephemeral: true }).catch(() => {}); }
});
