...
async function logMessageUpdate(oldMessage, newMessage) {
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      { name: '💬 Avant', value: (oldMessage.content || '*[Média / Embed]*').slice(0, 1024), inline: false },
      { name: '💬 Après', value: (newMessage.content || '*[Média / Embed]*').slice(0, 1024), inline: false }
    )
    .setFooter({ text: `ID Auteur : ${oldMessage.author.id}` })
    .setTimestamp();
...
async function logMessageDelete(message) {
  ...
  const embed = new EmbedBuilder()
    .setColor(0xEF4444)
    .setAuthor({ name: "🗑️ Suppression d'un message", iconURL: message.author?.displayAvatarURL({ dynamic: true }) || message.guild.iconURL() })
    .setDescription(`**Auteur :** ${message.author || 'Inconnu'}\n**Salon :** ${message.channel}\n\n**Contenu supprimé :**\n> ${(message.content || '*[Aucun contenu textuel / Média]*').slice(0, 1500)}`)
    .setFooter({ text: message.author ? `ID Auteur : ${message.author.id}` : '' })
    .setTimestamp();

  ch.send({ embeds: [embed] }).catch(() => {});
}

// ── 2. LOGS SERVEUR (Salons, Rôles, Pseudos) ──
...
async function logVoiceStateUpdate(oldState, newState) {
  ...
  if (!oldState.channel && newState.channel) {
    const embed = new EmbedBuilder()
      .setColor(0x10B981) // Vert émeraude
      .setAuthor({ name: "🔊 Connexion à un salon vocal", iconURL: member.user.displayAvatarURL({ dynamic: true }) })
  ...
  if (oldState.channel && !newState.channel) {
    const embed = new EmbedBuilder()
      ...
      .setAuthor({ name: "🔇 Déconnexion d'un salon vocal", iconURL: member.user.displayAvatarURL({ dynamic: true }) })
  ...
  if (oldState.channel && newState.channel && oldState.channelId !== newState.channelId) {
    const embed = new EmbedBuilder()
      .setColor(0x3B82F6) // Bleu pro
      .setAuthor({ name: "🔀 Déplacement en salon vocal", iconURL: member.user.displayAvatarURL({ dynamic: true }) })
      .setDescription(`**Membre :** ${member} (\`${member.user.tag}\`)\n\n• **Départ :** \`${oldState.channel.name}\`\n• **Arrivée :** \`${newState.channel.name}\``)
      .setTimestamp();
    return ch.send({ embeds: [embed] }).catch(() => {});
  }
}
