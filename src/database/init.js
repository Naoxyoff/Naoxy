...
async function initDatabase() {
  await db.execute(`
    CREATE TABLE IF NOT EXISTS guild_settings (
      ...
      an_kick_thresh INTEGER DEFAULT 5,
  ...
  await db.execute(`CREATE TABLE IF NOT EXISTS invite_stats (guild_id TEXT NOT NULL, inviter_id TEXT NOT NULL, count INTEGER DEFAULT 0, PRIMARY KEY (guild_id, inviter_id))`);

  await db.execute(`CREATE TABLE IF NOT EXISTS member_levels (guild_id TEXT NOT NULL, user_id TEXT NOT NULL, xp INTEGER DEFAULT 0, messages INTEGER DEFAULT 0, last_xp INTEGER DEFAULT 0, PRIMARY KEY (guild_id, user_id))`);

  await db.execute(`CREATE TABLE IF NOT EXISTS premium_subscriptions (id INTEGER PRIMARY KEY AUTOINCREMENT, guild_id TEXT, user_id TEXT, plan TEXT, type TEXT NOT NULL, status TEXT DEFAULT 'active', paypal_order_id TEXT, price REAL, started_at INTEGER DEFAULT (unixepoch()), expires_at INTEGER)`);

  // ── Migrations pour les bases déjà existantes (ALTER TABLE, ignorées si colonne déjà présente) ──
  const migrations = [
    `ALTER TABLE guild_settings ADD COLUMN invite_log_channel TEXT`,
    `ALTER TABLE guild_settings ADD COLUMN ai_enabled INTEGER DEFAULT 0`,
    `ALTER TABLE guild_settings ADD COLUMN ai_channel TEXT`,
    `ALTER TABLE guild_settings ADD COLUMN ai_model TEXT DEFAULT 'llama-3.3-70b-versatile'`,
    `ALTER TABLE guild_settings ADD COLUMN ai_max_tokens INTEGER DEFAULT 500`,
    `ALTER TABLE guild_settings ADD COLUMN ai_persona TEXT DEFAULT 'Orbis'`,
    `ALTER TABLE guild_settings ADD COLUMN antinuke_enabled INTEGER DEFAULT 0`,
    ...
    `ALTER TABLE guild_settings ADD COLUMN an_kick_thresh INTEGER DEFAULT 5`,
