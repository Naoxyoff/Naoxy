...
module.exports = (client, app) => {
  ...
  router.get("/api/guild/:guildId/roles", requireAuth, requireGuildAccess, (req, res) => {
    res.json(req.guild.roles.cache.filter(r => r.id !== req.guild.id).sort((a, b) => b.position - a.position).map(r => ({ id: r.id, name: r.name, color: r.hexColor, memberCount: r.members.size, position: r.position, mentionable: r.mentionable, hoist: r.hoist })));
  });
  ...
  router.get("/api/guild/:guildId/channels", requireAuth, requireGuildAccess, (req, res) => {
    res.json(req.guild.channels.cache.map(c => ({ id: c.id, name: c.name, type: c.type, parentId: c.parentId, parentName: c.parent?.name || null, position: c.rawPosition })).sort((a, b) => a.position - b.position));
  });

  router.get("/api/guild/:guildId/warnings", requireAuth, requireGuildAccess, (req, res) => {
    res.json(db.prepare("SELECT * FROM warnings WHERE guild_id = ? ORDER BY created_at DESC LIMIT 50").all(req.guild.id));
  });
  ...
  router.get("/api/guild/:guildId/sanctions", requireAuth, requireGuildAccess, (req, res) => {
    res.json(db.prepare("SELECT * FROM sanctions WHERE guild_id = ? ORDER BY created_at DESC LIMIT 50").all(req.guild.id));
  });

  router.get("/api/guild/:guildId/tickets", requireAuth, requireGuildAccess, (req, res) => {
    res.json(db.prepare("SELECT * FROM tickets WHERE guild_id = ? ORDER BY created_at DESC LIMIT 50").all(req.guild.id));
  });
  ...
  router.patch("/api/guild/:guildId/tickets/:id/close", requireAuth, requireGuildAccess, async (req, res) => {
    ...
    try { const ch = req.guild.channels.cache.get(ticket.channel_id); if (ch) await ch.delete("Ticket fermé via le dashboard"); } catch (_) {}
    res.json({ success: true });
  });
  ...
  router.get("/api/guild/:guildId/autoroles", requireAuth, requireGuildAccess, (req, res) => {
    res.json(db.prepare("SELECT * FROM autoroles WHERE guild_id = ?").all(req.guild.id));
  });
  ...
  router.get("/api/guild/:guildId/commands", requireAuth, requireGuildAccess, (req, res) => {
    res.json(db.prepare("SELECT * FROM custom_commands WHERE guild_id = ?").all(req.guild.id));
  });
  ...
  router.post("/api/guild/:guildId/commands", requireAuth, requireGuildAccess, (req, res) => {
    ...
    try { db.prepare("INSERT INTO custom_commands (guild_id, name, response, created_by) VALUES (?, ?, ?, ?)").run(req.guild.id, name.toLowerCase(), response, req.session.user.id); res.json({ success: true }); } catch (e) { res.status(400).json({ error: "Commande déjà existante" }); }
  });
  ...
  router.post("/api/guild/:guildId/channels/:channelId/send", requireAuth, requireGuildAccess, async (req, res) => {
    try {
      ...
      const msg = req.body.embed ? await ch.send({ embeds: [{ title: req.body.title, description: req.body.content, color: 0x7c3aed }] }) : await ch.send(req.body.content);
  ...
  router.get("/api/guild/:guildId/bans", requireAuth, requireGuildAccess, async (req, res) => {
    try { const bans = await req.guild.bans.fetch(); res.json(bans.map(b => ({ userId: b.user.id, username: b.user.username, avatar: b.user.displayAvatarURL({ size: 64 }), reason: b.reason }))); } catch (e) { res.status(400).json({ error: e.message }); }
  });

  router.post("/api/guild/:guildId/bans/:userId/unban", requireAuth, requireGuildAccess, async (req, res) => {
    try { await req.guild.members.unban(req.params.userId, req.body.reason || "Débanni via le dashboard"); res.json({ success: true }); } catch (e) { res.status(400).json({ error: e.message }); }
  });
  ...
  router.get('/api/guild/:guildId/audit-log', requireAuth, requireGuildAccess, async (req, res) => {
    try { const logs = await req.guild.fetchAuditLogs({ limit: 20 }); res.json(logs.entries.map(e => ({ id: e.id, action: e.action, executorId: e.executor?.id, targetId: e.target?.id, reason: e.reason, createdAt: e.createdAt }))); } catch (e) { res.json([]); }
  });
